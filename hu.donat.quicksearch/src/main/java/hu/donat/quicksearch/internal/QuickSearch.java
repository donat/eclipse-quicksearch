...
/**
 * <b><tt>Taken from the Buildship project (licensed under EPL v1.0). Source:
 * <a href="https://github.com/eclipse/buildship/blob/3463c54bcbea3724a11ad75a8171a4a9897d7b1b/org.eclipse.buildship.ui/src/main/java/org/eclipse/buildship/ui/view/task/QuickSearchManager.java">http://github.com/eclipse/buildship
 * </a></tt></b>
 * <p>
...
public final class QuickSearch {
    ...
    private boolean traverseExpandedItemsOnly = false;

    public QuickSearch(Tree tree, OverlayLabel label) {
    ...
    private void performSearch() {
        ...
        final String searchText = QuickSearch.this.state.searchText.toUpperCase();
    ...
    private void filterRecursively(ImmutableList<TreeItem> items, Predicate<TreeItem> predicate, ImmutableList.Builder<TreeItem> hits) {
        for (TreeItem item : items) {
            ...
            if (!this.traverseExpandedItemsOnly || item.getExpanded()) {
    ...
    private void updateLabel() {
        ...
        if (isSearchActive()) {
            this.label.updateText(String.format("%s (%d matches)", this.state.searchText, this.state.results.length));
        } else {
            this.label.updateText(null);
        }
    }
